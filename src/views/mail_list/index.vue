<template>
  <div class="app-container calendar-list-container">
    ...
    >
      ...
      <el-table-column class="status-col" label="Types of" width="90px">
        <template scope="scope">
          <el-tag :type="scope.row.type | typeFilter">{{scope.row.type | typeShowFilter}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" label="Sender">
        <template scope="scope">
      ...
      <el-table-column label="Subject" :show-overflow-tooltip="true" min-width="500px">
        <template scope="scope">
      ...
      <el-table-column prop="date" sortable="custom" align="center" label="Time" width="200px">
        <template scope="scope">
...
export default {
  ...
  methods: {
    ...
    handleDelete() {
      ...
      this.$confirm('Are you sure want to delete this' + selectedLen + ' Mail?', 'Confirm', {
        ...
        .then(() => {
          ...
          // When deleting, the business may require a type, and the type is multipleSelection.type
    ...
    unDeleted() {
      const selectedLen = this.multipleSelection.length || 0;
      if (selectedLen < 1) {
        this.$message('Please select mail to undo delete');
        return;
      }
      const idArr = [];
      this.multipleSelection.forEach(item => idArr.push(item.id));
      // Business may require a type, type at multipleSelection.type
    ...
    handleDownload() {
      require.ensure([], () => {
        ...
        const tHeader = ['Sender', 'Sending mailbox', 'Subject', 'time'];
        const filterVal = ['sendName', 'sendMail', 'title', 'date'];
        const data = this.formatJson(filterVal, this.list);
        export_json_to_excel(
          tHeader,
          data,
          parseTime(Date.now()) + 'Mailing list data'
        );
      });
    },
