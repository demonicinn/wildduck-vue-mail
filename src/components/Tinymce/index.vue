...
// import { getToken, upload } from 'api/qiniu'; // Seven cattle
// Business needs can be deleted
export default {
  name: "tinymce",
  components: { editorImage, editorAudio, editorSlide, editorVideo }, // Business needs can be deleted
  props: {
    id: {
      type: String,
  ...
  mounted() {
    ...
    tinymce.init({
      ...
      block_formats: "Normal label = p; subtitle = h2;",
  ...
  methods: {
    /* Business code can be deleted start*/
    imageSuccessCBK(arr) {
      console.log(arr);
      const _this = this;
      arr.forEach(v => {
    ...
    /* Business code can be deleted end*/
  },
  destroyed() {
